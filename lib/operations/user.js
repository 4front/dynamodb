...
module.exports = function(DynamoDb) {
  ...
  DynamoDb.prototype.getUserInfo = function(userIds, callback) {
    ...
    this.models.User.getItems(userIds, {
      AttributesToGet: ['userId'].concat(attrs)
    }, function(err, data) {
  ...
  DynamoDb.prototype.listUserOrgs = function(userId, callback) {
    ...
    self.models.OrgMember
      ...
      .exec(function(err, data) {
        ...
        self.models.Organization.getItems(_.map(userOrgs, 'orgId'), function(innerErr, orgData) {
          if (err) return callback(innerErr);

          var orgs = _.reject(_.map(orgData, 'attrs'), {terminated: true});
