...
module.exports = function(DynamoDb) {
  ...
  DynamoDb.prototype.deleteLegacyDomain = function(domainName, callback) {
    this.models.LegacyDomain.destroy(domainName, callback);
  };
  ...
  DynamoDb.prototype.updateDomain = function(domainData, callback) {
    debug('update domain %s', domainData.domainName);

    domainData = _.pick(domainData, 'domainName', 'orgId',
      'status', 'dnsValue', 'cdnDistributionId', 'catchAllRedirect');

    this.models.Domain.update(domainData,
