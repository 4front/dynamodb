...
module.exports = function(DynamoDb) {
  ...
  DynamoDb.prototype.createApplication = function(appData, callback) {
    ...
    async.series([
      function(cb) {
        if (_.isEmpty(appData.domainName)) return cb();
        ...
        // debugger;
        self.getAppIdByDomainName(appData.domainName, appData.subDomain, function(err, appId) {
          if (err) return cb(err);
          if (_.isEmpty(appId) === false) {
            return cb(Error.create('Domain name ' + appData.subDomain + '.' + appData.domainName + ' already taken.',
    ...
    async.parallel([
      function(cb) {
        self.models.Application.get(appId, function(err, data) {
          if (err) {
            return cb(Error.create('Error fetching application from Dynamo', {
              code: err.code,
  ...
  // Get the appId for a domainName and subDomain
  DynamoDb.prototype.getAppIdByDomainName = function(domainName, subDomain, callback) {
    this.models.Application.query(domainName, subDomain)
      .usingIndex('domainNameIndex')
      .where('subDomain').equals(subDomain)
      .exec(function(err, data) {
        if (err) return callback(err);
        if (data.Items.length === 0) return callback(null, null);
        callback(null, data.Items[0].attrs.appId);
      });
  };
  ...
  DynamoDb.prototype.updateApplication = function(appData, callback) {
    ...
    async.series([
      function(cb) {
        ...
        var updateAttrs = _.pick(appData, 'appId', 'orgId',
          'requireSsl', 'trafficControlEnabled', 'addOnSettings',
