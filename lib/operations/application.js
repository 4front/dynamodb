	  ...
	  async.parallel([
	    function(cb) {
	      self.models.Application.get(appId, function(err, data) {
	        if (err)
	          return cb(Error.create("Error querying applications table", {appId: appId}, err));
	    ...
	    function(cb) {
	      ...
	      self.models.AppName.query(appId).usingIndex('appIdIndex').exec(function(err, data) {
	        if (err)
	          return cb(Error.create("Error querying appName", {appId: appId}, err));
	    ...
	    function(cb) {
	      ...
	      self.models.Domain.query(appId).usingIndex('appIdIndex').exec(function(err, data) {
	        if (err)
	          return cb(Error.create("Error querying domains table", {appId: appId}, err));
