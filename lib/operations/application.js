...
module.exports = function(DynamoDb) {
	        ...
	        if (!data) {
	          ...
	          return cb(null);
					...
					if (_.isObject(data.attrs.env)) {
						_.each(data.attrs.env, function(map, envName) {
							_.each(map, function(value, key) {
								// Decrypt the value in place.
								if (value.encrypted === true)
									value.value = crypto.decrypt(value.value, self.cryptoPassword);
							});
						});
					}
	  ...
	  var updatedApp = {};
		...
		async.series([
			function(cb) {
				...
				// TODO: Deprecate some of these attributes
				var updateAttrs = _.pick(appData, 'appId', 'orgId', 'configSettings',
					'authConfig', 'requireSsl', 'snapshotsEnabled', 'trafficControlEnabled');
	...
	DynamoDb.prototype.getDomain = function(domain, callback) {
		this.models.Domain.get(domain, this._itemCallback(callback));
	};

	DynamoDb.prototype.updateDomains = function(appId, domainNames, callback) {
		var self = this;

		debug("update domains to %s", domainNames);
		this.models.Domain.query(appId).usingIndex('appIdIndex').exec(function(err, data) {
	    if (err) return callback(err);
	    ...
	    var newDomains = _.difference(domainNames, existingDomains);
	    var deletedDomains = _.difference(existingDomains, domainNames);
	    ...
	    if (newDomains.length > 0) {
	      ...
	      tasks.push(function(cb) {
	        async.each(newDomains, function(domain, cb1) {
						self.createDomain(appId, domain, cb1);
	        }, cb);
	      });
	    }
	...
	DynamoDb.prototype.updateDomainZone = function(domainName, zone, callback) {
		this.models.Domain.update({domain: domainName, zone: zone}, this._itemCallback(callback));
	};
};
