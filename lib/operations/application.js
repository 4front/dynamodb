...
module.exports = function(DynamoDb) {
					...
					if (_.isObject(data.attrs.env)) {
						_.each(data.attrs.env, function(map, envName) {
							_.each(map, function(value, key) {
								...
								map[key] = crypto.decrypt(value, self.cryptoPassword);
							});
						});
					}
	...
	DynamoDb.prototype.setEnvironmentVariable = function(options, callback) {
		...
		if (options.encrypt === true)
			options.value = crypto.encrypt(options.value, this.cryptoPassword);
		...
		this.models.Application.update({appId:options.appId}, params, function(err) {
			...
			params = {
				...
				ExpressionAttributeValues: {':value': options.value}
			};
